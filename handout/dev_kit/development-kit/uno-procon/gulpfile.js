...
function executeTest(done) {
  ...
  files = files ? 'test/scripts/s00.pre.spec.{ts,js} ' + files.trim().replace(',', ' ') : 'test/scripts/**/*.spec.{ts,js}';
  // files = files ? 'src/**/*.test.ts' + files.trim().replace(',', ' ') : 'src/**/*.test.ts';

  shell.task([
    `${windows ? 'cross-env NODE_ENV=test ' : 'cross-env NODE_ENV=test '} TS_NODE_FILES=true ${reports ? 'nyc' : ''} mocha `
