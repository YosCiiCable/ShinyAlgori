...
describe('Dealer', () => {
  it(`s01-A-TC001: Admin create Dealer 1 - successfully`, (done) => {
    const body = { name: Consts.DEALER_1, totalTurn: Consts.TOTAL_TURN };
    request
      .post('')
      .send(body)
  ...
  it(`s01-A-TC002: Admin create more Dealer 1 - failed - Reason: Dealer exists with same name`, (done) => {
    const body = { name: Consts.DEALER_1, totalTurn: Consts.TOTAL_TURN };
    request.post('').send(body).expectStatus(400).end(done);
  });
  ...
  it(`s01-A-TC004: Admin create more Dealer 1 - failed - Reason: Total Turn is required`, (done) => {
    const body = { name: Consts.DEALER_1 };
    request.post('').send(body).expectStatus(422).end(done);
  });
